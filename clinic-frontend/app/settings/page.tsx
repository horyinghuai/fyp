...
export default function SettingsPage() {
  ...
  useEffect(() => {
      const fetchProfile = async () => {
          ...
          if (!token) return;
          try {
              const res = await fetch('http://127.0.0.1:8000/admin/profile', { headers: { 'Authorization': `Bearer ${token}` } });
              if (res.ok) {
  ...
  const proceedWithUpdate = async (emailAlreadyUpdated: boolean) => {
    ...
    try {
        ...
        const res = await fetch(`http://127.0.0.1:8000/admin/profile`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify(payload) });
        if (res.ok) {
            const data = await res.json();
            const userStr = localStorage.getItem('aicas_user');
            if(userStr) {
                const user = JSON.parse(userStr);
                user.name = data.name; user.email = formData.email;
    ...
    } catch (e) { setStatus({ type: 'error', text: 'Server error.' }); }
    setIsLoading(false);
  };
  ...
  const handleRequestEmailChange = async (isResend = false) => {
      ...
      try {
          ...
          if (res.ok) {
              setEmailPhase('code');
              setResendTimer(15);
          } else { setModalError(data.detail || 'Failed to request change.'); }
      } catch (err) { setModalError('Server error. Please try again.'); }
      setIsLoading(false);
  };
  ...
  const handleVerifyEmailChange = async () => {
      ...
      try {
          ...
          if (res.ok) {
              ...
              window.dispatchEvent(new Event('storage'));
          } else { setModalError(data.detail || 'Invalid or expired code.'); }
      } catch (err) { setModalError('Server error. Please try again.'); }
      setIsLoading(false);
  };
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {status.text && ( <div className={`p-4 rounded-xl mb-6 font-medium ${status.type === 'error' ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'}`}>{status.text}</div> )}
        ...
        <form onSubmit={handleUpdate} className="space-y-6">
          <div className="grid grid-cols-1 gap-6">
            <div className="w-full">
              ...
              <input type="text" value={formData.name || ''} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full p-3 border rounded-xl outline-none focus:ring-2 focus:ring-blue-500 bg-slate-50 uppercase" required />
            </div>

            <div>
              <label className="block text-sm font-bold text-slate-700 mb-2">Email Address (Login ID)</label>
              <div className="flex gap-3">
                  <input type="email" value={formData.email || ''} readOnly className="w-[70%] p-3 border rounded-xl outline-none bg-slate-100 text-slate-500 cursor-not-allowed" />
                  <button type="button" onClick={() => { setShowEmailModal(true); setEmailPhase('password'); setNewEmailTarget(''); setCurrentPassword(''); setVerifyCode(''); setModalError(''); }} className="w-[30%] p-3 bg-slate-200 text-slate-700 font-bold rounded-xl hover:bg-slate-300 transition whitespace-nowrap">
                      Change Email
                  </button>
              </div>
            </div>
          ...
          <div className="border-t border-slate-100 pt-6 mt-6">
             ...
             <div className="grid grid-cols-2 gap-6 mb-6">
                <div><label className="block text-sm font-bold text-slate-700 mb-2">New Password</label><input type="password" placeholder="••••••••" value={formData.newPassword || ''} onChange={(e) => setFormData({...formData, newPassword: e.target.value})} className="w-full p-3 border rounded-xl outline-none focus:ring-2 focus:ring-blue-500" /></div>
                <div><label className="block text-sm font-bold text-slate-700 mb-2">Confirm New Password</label><input type="password" placeholder="••••••••" value={formData.confirmPassword || ''} onChange={(e) => setFormData({...formData, confirmPassword: e.target.value})} className="w-full p-3 border rounded-xl outline-none focus:ring-2 focus:ring-blue-500" /></div>
             </div>
      ...
      {showEmailModal && (
        <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 backdrop-blur-sm">
            <div className="bg-white p-8 rounded-2xl w-[450px] shadow-2xl">
                ...
                {modalError && ( <div className="p-3 rounded-lg mb-4 text-sm font-bold bg-red-100 text-red-700">{modalError}</div> )}
                
                {emailPhase === 'password' ? (
                    <>
                        <div className="mb-4">
                            <label className="block text-sm font-bold text-slate-700 mb-1">Current Email</label>
                            <input type="email" readOnly value={originalEmail} className="w-full p-3 border rounded-xl bg-slate-100 text-slate-500 cursor-not-allowed" />
                        </div>
                        <div className="mb-4">
                            <label className="block text-sm font-bold text-slate-700 mb-1">New Email Address</label>
                            <input type="email" placeholder="Enter your new email address" value={newEmailTarget} onChange={e => setNewEmailTarget(e.target.value)} className="w-full p-3 border rounded-xl outline-none focus:ring-2 focus:ring-blue-500" />
                        </div>
                        <div className="mb-6">
                            <label className="block text-sm font-bold text-slate-700 mb-1">Current Password</label>
                            <input type="password" placeholder="Enter your current password" value={currentPassword} onChange={e => setCurrentPassword(e.target.value)} className="w-full p-3 border rounded-xl outline-none focus:ring-2 focus:ring-blue-500" />
                        </div>
                    ...
                    <>
                        ...
                        <div className="mb-2">
                            ...
                            <input type="text" placeholder="------" value={verifyCode} onChange={e => setVerifyCode(e.target.value)} maxLength={6} className="w-full p-4 border rounded-xl outline-none focus:ring-2 focus:ring-emerald-500 font-mono tracking-[1em] text-center text-2xl" />
                        </div>
                        <div className="text-sm text-center mb-6">
                            <span className="text-slate-500">Didn't receive the code? </span>
                            {resendTimer > 0 ? ( <span className="font-bold text-slate-400">Resend in {resendTimer}s</span> ) : ( <button type="button" onClick={() => handleRequestEmailChange(true)} disabled={isLoading} className="font-bold text-blue-600 hover:text-blue-700 hover:underline">Resend Code</button> )}
                        </div>
                        <div className="flex gap-3">
                            <button onClick={() => setShowEmailModal(false)} className="flex-1 bg-slate-100 text-slate-700 font-bold py-3 rounded-xl hover:bg-slate-200 transition">Cancel</button>
