  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        ...
        <button onClick={() => openModal()} className="px-4 py-2 bg-emerald-600 text-white rounded-lg font-bold hover:bg-emerald-700 shadow-md">+ Add Blood Test</button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        {packages.map(p => (
          <div key={p.id} className="bg-white rounded-xl p-6 shadow-sm border-t-4 border-emerald-500 flex flex-col hover:shadow-md transition">
            <div className="flex justify-between mb-2">
              <h3 className="font-bold text-lg text-slate-800">{p.name}</h3>
              <span className="font-black text-emerald-600">RM {p.price}</span>
            </div>
            <p className="text-sm text-slate-500 mb-4 italic">{p.description}</p>
            <div className="mt-auto flex justify-end gap-2 border-t pt-4">
              <button onClick={() => openModal(p)} className="text-sm px-4 py-2 bg-slate-100 text-slate-600 font-medium rounded-lg hover:bg-slate-200 transition">Edit</button>
              <button onClick={() => handleDelete(p.id)} className="text-sm px-4 py-2 bg-red-50 text-red-600 font-medium rounded-lg hover:bg-red-100 transition">Delete</button>
            </div>
          </div>
        ))}
      </div>
            ...
            {singles.map(s => (
              <tr key={s.id} className="border-b border-slate-50 hover:bg-slate-50 transition">
                ...
                <td className="p-4 text-center space-x-2">
                  <button onClick={() => openModal(s)} className="text-sm px-3 py-1 bg-slate-100 text-slate-600 rounded font-medium hover:bg-slate-200">Edit</button>
                  <button onClick={() => handleDelete(s.id)} className="text-sm px-3 py-1 bg-red-50 text-red-600 rounded font-medium hover:bg-red-100">Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 backdrop-blur-sm">
          <div className="bg-white p-6 rounded-2xl w-[400px] shadow-2xl">
            ...
            <div className="space-y-3">
              ...
              <input type="number" placeholder="Price (RM)" value={formData.price || ''} onChange={e => setFormData({...formData, price: parseFloat(e.target.value) || 0})} className="w-full p-3 border border-slate-200 rounded-lg outline-none focus:border-emerald-500" />
              <select value={formData.test_type} onChange={e => setFormData({...formData, test_type: e.target.value})} className="w-full p-3 border border-slate-200 rounded-lg outline-none focus:border-emerald-500 bg-white">
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button onClick={handleSave} className="px-4 py-2 bg-emerald-600 text-white rounded-lg font-medium hover:bg-emerald-700 transition">Save Test</button>
            </div>
          </div>
        </div>
      )}
